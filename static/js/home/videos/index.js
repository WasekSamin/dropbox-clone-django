function showVideoFilePreview(e) {
    ...
    if (videoEl) {
        const videoSrc = window.location.origin + videoEl.querySelector("source")?.getAttribute("src");
...
function removeVideoElement() {
    const videoEl = document.getElementById("video__previewModal")?.querySelector("video");
    videoEl?.remove();
